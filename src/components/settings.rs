...
use crate::{components::rem, game::{GameState, HonorSkin, ScreenState, SuitSkin}};
...
pub fn Settings(game_state: Signal<GameState>) -> Element {
    ...
    let honor_skin_changed = move |evt: Event<FormData>| {
        let v = evt.value().parse().ok().and_then(|v| { HonorSkin::from_repr(v) });
        state.write().skin.honors = v.unwrap_or_default();
    };
        ...
        div {
            ...
            div {
                ...
                p {
                    "Honors: "
                    select {
                        onchange: honor_skin_changed,
                        for x in HonorSkin::iter() {
                            option {
                                value: x as usize,
                                selected: state.read().skin.honors == x,
                                "{x}"
                            }
                        }
                    }
            ...
            p {
                class: "copyright",
                "Game rules: “Shenzhen Solitaire” by Zachtronics", br{},
